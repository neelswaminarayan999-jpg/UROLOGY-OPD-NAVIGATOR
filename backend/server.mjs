...
const FALLBACK_MODELS = String(process.env.GEMINI_FALLBACK_MODELS || 'gemini-3.7-flash,gemini-3.5-flash,gemini-3.5-flash-lite').split(',').map(s => s.trim()).filter(Boolean);
const MAX_MODEL_ATTEMPTS = Math.max(1, Number(process.env.GEMINI_MAX_MODEL_ATTEMPTS || 4));
const MAX_RETRIES_PER_MODEL = Math.max(0, Number(process.env.GEMINI_RETRIES_PER_MODEL || 2));
...
function retryDelayMs(attempt, retryAfterHeader='') {
  ...
  if (Number.isFinite(h) && h >= 0) return Math.min(15000, h * 1000);
  return Math.min(8000, 900 * (2 ** attempt) + Math.floor(Math.random() * 250));
}
async function requestGeminiModel(model, request) {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES_PER_MODEL; attempt++) {
    ...
    const msg = data?.error?.message || txt.slice(0, 1000) || `Gemini HTTP ${r.status}`;
    lastError = Object.assign(new Error(`Gemini API (${model}): ${msg}`), { status: r.status, retryable: retryableStatus(r.status) });
    if (!retryableStatus(r.status) || attempt >= MAX_RETRIES_PER_MODEL) break;
    await sleep(retryDelayMs(attempt, r.headers.get('retry-after') || ''));
  }
  throw lastError || new Error(`Gemini API (${model}) failed`);
}
async function callGemini(body) {
  ...
  const candidates = [MODEL, ...FALLBACK_MODELS].filter((m, i, a) => m && a.indexOf(m) === i).slice(0, MAX_MODEL_ATTEMPTS);
  const errors = [];
  for (const model of candidates) {
    try {
      const { data } = await requestGeminiModel(model, request);
      const output_text = extractGeminiText(data);
      if (!output_text) {
        const err = new Error(`Gemini API (${model}) returned no text.`);
      ...
      return {
        ...
        model,
        provider: 'Gemini',
        failover_used: model !== MODEL,
        failover_note: model !== MODEL ? `Primary model ${MODEL} was temporarily unavailable; response obtained from ${model}.` : ''
      };
    } catch (e) {
  ...
  throw new Error(`Gemini is temporarily unavailable. Automatic retries and fallback models were attempted. ${errors[errors.length - 1] || 'Please try again.'}`);
}
